...
char *test_name = "test_commit_action";
int seq_num = 0;	
int retries;
int score; 
...
void 
test2and3_commit_action_1(unsigned int numParams, unsigned int *params)
{
	assert(numParams == 1); 
...
void 
test2and3_commit_action_2(unsigned int numParams, unsigned int *params)
{
	assert(numParams == 1); 
	int my_seq_num = (int) params[0];
...
test2() 
{
	...
	XACT_BEGIN(xact_commit_action1)
		...
		XACT_BEGIN(xact_commit_action2)
			...
			txc_register_commit_action(test2and3_commit_action_1, 1, inputarray);
			__tm_waiver {
				if (retries++ == 0) {
					XACT_RETRY
				}
		...
		__tm_waiver { seq_num++; }
		inputarray[0] = (unsigned int) seq_num;
		txc_register_commit_action(test2and3_commit_action_2, 1, inputarray);
	XACT_END(xact_commit_action1)
	if (score == 3888) {
